...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  },
  {
    ...
    methods: [
      ...
      {
        signature: 'setTheme(id: string): Promise<boolean>',
        ...
        parameters: [{ name: 'id', description: 'a registered theme id or `system`; unknown ids throw synchronously.' }],
        returns: 'settlement after the write, resolving whether the settled state kept the id; a Host refusal reloads the durable value and settles `false`.',
        throws: ['an unknown theme id, synchronously.'],
      },
      {
        signature: 'setFontSize(px: number): Promise<boolean>',
        ...
        parameters: [{ name: 'px', description: 'integer px within FONT_SIZE_MIN..FONT_SIZE_MAX; out-of-range or fractional values throw synchronously.' }],
        returns: 'settlement after the write, resolving whether the settled state kept the size; a Host refusal reloads the durable value and settles `false`.',
        throws: ['an out-of-range or fractional size, synchronously.'],
      },
      {
        signature: 'setFontFamily(family: FontFamily): Promise<boolean>',
        description: 'Select the reading font family — the only write entry for it. Accepted values are written through the settings scope and emit `theme/change`.',
        parameters: [{ name: 'family', description: 'a value from FONT_FAMILIES; anything else throws synchronously.' }],
        returns: 'settlement after the write, resolving whether the settled state kept the family.',
        throws: ['an unsupported family id, synchronously.'],
      },
      {
        signature: 'setWideSpacing(wide: boolean): Promise<boolean>',
        description: 'Turn the wider reading spacing on or off — the only write entry for it. The value is written through the settings scope and emits `theme/change`.',
        parameters: [{ name: 'wide', description: 'whether the conversation text carries the wider spacing.' }],
        returns: 'settlement after the write, resolving whether the settled state kept the choice.',
      },
      {
